...
for i in $( cat ${dirs} ); do
    ...
    point2dem --threads 16 --t_srs "${proj}" -r mars --nodata -32767 -s 24 ${i}_map_ba_align-trans_reference.tif --orthoimage -n --errorimage ../${i}_map_ba-L.tif -o ${i}_map_ba_align_24
    if [ $? -ne 0 ]
    then
    ...
    dem_geoid --threads 16 ${i}_map_ba_align_24-DEM.tif -o ${i}_map_ba_align_24-DEM
    if [ $? -ne 0 ]
    then
