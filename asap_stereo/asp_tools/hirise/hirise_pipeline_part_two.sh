...
function step1() {
    asp_hirise_pc_align2dem.sh -d $1 -m $2 -r $3
    if [ $? -eq 0 ]
    then
...
print_usage (){
    echo ""
    echo "Usage: $0 <stereodirs.lis> <max-displacement> <reference-dem>"
    ...
    echo " and <reference-dem> is the DEM (not adjusted using dem_geoid), including path, to align the HiRISE point cloud to."
